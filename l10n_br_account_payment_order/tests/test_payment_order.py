...
class TestPaymentOrder(TransactionCase):
    ...
    def test_implemented_fields_payment_order(self):
        ...
        assert (
            self.invoice_customer_original.payment_mode_id
        ...
        for line in self.invoice_customer_original.move_id.line_ids.filtered(
            lambda l: l.account_id.id == self.invoice_customer_original.account_id.id
        ):
            self.assertEquals(
        ...
        self.assertEquals(test_balance_value, 300.0, "Error with method get_balance()")
    ...
    def test_cancel_payment_order(self):
        ...
        for line in payment_order.payment_line_ids:
            ...
            self.assertEquals(
                line._get_info_partner(self.invoice_customer_original.partner_id),
                "AKRETION LTDA",
                "Error with method _get_info_partner",
            )
            test_amount_interest = line.amount_interest
        self.assertEquals(
            test_amount_interest, 4.5, "Error with compute field amount_interest."
        )
